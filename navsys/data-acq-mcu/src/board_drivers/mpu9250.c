...
void IMU_2_Setup(int div) 
{
	...
	/* установили делитель */
	SET_BIT(SPI4->CR1, SPI_CR1_BR | (a << POSITION_VAL(SPI_CR1_BR)));
	
	/* разрешаем прерывание по приёму и передаче */
	SET_BIT(SPI4->CR2, SPI_CR2_RXNEIE | SPI_CR2_TXEIE);
...
{
	imu2.active_register = reg;		/* Какой регистр мы читали? */
	imu2.txstate = TXSTATE_IDLE;	/* Надо ли сразу же что-то передать? Нет. */
	SPI4->DR = REG_READ(reg);		/* Отправляем запрос на чтение */
}

void IMU_2_WriteCommand(MPU9250_REG_ADDRn reg, char data)
{
	imu2.txdata = data;
	imu2.active_register = reg;
	imu2.txstate = TXSTATE_SEND_NEXT_DATA;
	SPI4->DR = REG_WRITE(reg);
}

void IMU_2_InterruptHandler()
{
	if (READ_BIT(SPI4->SR, SPI_SR_RXNE)) {
		imu2.rxdata = SPI4->DR;
		
		switch (imu2.active_register) {
			case RA_WHO_AM_I: {
				assert(imu2_rx_buf == 0x68 || imu2_rx_buf == 0x71, "mpu9520 #2 comm failure");
				break;
			}
		}
	...
	if (READ_BIT(SPI4->SR, SPI_SR_TXE)) {
		if (imu2.txstate == TXSTATE_SEND_NEXT_DATA) {
			SPI4->DR = imu2.txdata;
			imu2.txstate = TXSTATE_IDLE;
		}
	}
}
